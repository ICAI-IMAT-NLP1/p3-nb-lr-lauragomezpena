...
class NaiveBayes:
    ...
    def estimate_class_priors(self, labels: torch.Tensor) -> Dict[int, torch.Tensor]:
        ...
        for i in range(len(unique_classes)):
            class_priors[unique_classes[i].item()] = (counts[i]/total_samples)
        return class_priors
    ...
    def estimate_conditional_probabilities(
        ...
        for label in unique_labels:
            ...
            words_label = features[labels == label]

            counts = words_label.sum(dim = 0)
            total_count = counts.sum() 

            class_word_counts[label_int] = (counts + delta)/(total_count+delta * n_words)
    ...
    def estimate_class_posteriors(
        ...
        class_labels = sorted(self.class_priors.keys())

        log_priors = torch.log(torch.tensor(
            [self.class_priors[c].item() for c in class_labels]
        ))
        ...
        for idx, c in enumerate(class_labels):
            ...
            log_posteriors[idx] += log_likelihoods
